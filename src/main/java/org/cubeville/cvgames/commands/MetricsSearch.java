...
public class MetricsSearch extends RunnableCommand {
    @Override
    public TextComponent execute(CommandSender sender, List<Object> params) throws Error {
        String searchText = (String) params.get(0);
        TextComponent output = new TextComponent("Metrics that match query \"" + searchText + "\":\n");
        output.setBold(true);
        output.setColor(ChatColor.of("#f2f2f2"));
        Map<MetricKey, Long> metricMap = CVGames.dataManager().getSearchMetrics(searchText);
